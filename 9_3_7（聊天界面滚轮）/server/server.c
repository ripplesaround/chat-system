...
typedef struct _space{
    ...
    //xzb
    Message allmsg[MAXMSG+1]; /*群聊消息*/
    int doculen;              /*当前文件数量*/
    Docu unsenddocu[MAXMSG+1];/*文件*/
    //xzb
...
char chat_with[20];           /*在进入单聊的时候标示*/
//struct                        /*保存当前的未读消息（在弹窗时的）*/
...
//fwx
int send_unread_message();
/*在write函数里写入未读消息*/

int send_unread_message()
{
    int fd, unread = 0;
    char path[20] = "history/";
    char buffer[10];
    //写文件名，to是fork_id ,但得知道是谁发给我的啊？？？
    int temp;
    temp = find(chat_with);
    ...
    Kind kind;
    if(unread>0)
        printf("test_send_unread 当前进程的id：%d unread:%d\n", fork_id, unread);
    //???
    for(int i=0;i<unread;++i)
    {
        read(fd, &data, sizeof(Data));
        printf("i=%d fasong: from:%s  to:%s str:%s\n",i, data.message.id_from,data.message.id_to,data.message.str);

        // strcpy(data.message.str,"ripples");
        // strcpy(data.message.id_to,"fwx");
        // strcpy(data.message.id_from,"lx");
        ...
        printf("sendmess发送包 %s 当前进程：%s\n",data.message.str,fwxnb);
    }        
    unread = 0;
    lseek(fd,0,SEEK_SET);
...
void write_all()
{
    ...
    while(1){
        ...
        temp = update_unread();                             /*在聊天界面接收未读消息*/
        if(msglength < space->length){                      /*有新消息，互斥访问共享内存*/
            ...
            {
                if(space->fwxnp[find(fwxnb)]==21&&strcmp(space->allmsg[msglength%MAXMSG].id_from,fwxnb)!=0)
                {
                    if(build_packet(&packet,enum_ipchat,space->allmsg[msglength%MAXMSG]) == -1)
                    {
                        printf("180 fail to build the packet!\n");
                        return;
                    }
                    printf("群聊消息，由%s发送\n",space->allmsg[msglength%MAXMSG].id_from);
...
void read_all()
{
    ...
    while(1) {
        ...
        if (read_byte == -1) {
            ...
            space->allmsg[msglength].id = space->length;
            strcpy(space->allmsg[msglength].str, data.message.str);
            strcpy(space->allmsg[msglength].id_from, data.message.id_from);
            printf("fork_id==广场==%d\n", fork_id);
            //服务器端加时间戳
            time(&t);
            ctime_r(&t, tbuf);
            ...
            printf("Client\"%s\" has writed the message %s .\n", data.message.id_from,
                   space->allmsg[msglength].str);
            if ((fp = fopen("gchist.txt", "a")) == NULL) {
                printf("gchist_txt_error\n");
            }
            else                                            /*将历史记录写入gchist.txt*/
            {
                fputs(data.message.id_from, fp);
                fprintf(fp, "--->");
                fputs(tbuf, fp);
                fputs(space->allmsg[msglength].str, fp);
                fprintf(fp, "\n\n");
                fclose(fp);
            }
            V(RW);                                          /*释放共享内存区信号量*/
...
void return_fri()
{
    ...
    for(int xxx=1;xxx<MAXUSER+1;++xxx)
    {
        ...
        {
            //printf("test id(%d)给id(%d)发送没有收到，需要马上发送\n",xxx,fork_id); //B返回给A
            int fd; 
...
int update_unread()
{
    ...
    Kind kind;
    if(unread>0)
        printf("test 当前进程的id：%d 系统公告unread:%d\n", fork_id,unread);
    //???
    for(int i=0;i<unread;++i)
    {
        read(fd, &data, sizeof(data));
    ...
    while(1){
        ...
        else
        {
            ...
            {                                                   /*写内存区*/
                ...
                int temp = find(data.message.id_to);            /*查找送给谁的id*/
                printf("fork_id %d\n",fork_id);
                printf("test read_from 读到的信息里要发送给人的id：%d ", temp); 
                printf("此人的状态：%d\n",space->fwxnp[temp]);
                
                if(space->fwxnp[temp] == 0)
                {
                    printf("此人(%d)在主界面\n",temp);
                    msglength=space->length%MAXMSG;                 /*更新内存区,超过500条则覆盖前面的，写的位置永远是space->length%500*/
                    //char buffer[140];
                    //strcpy(buffer,space->message[msglength].str);
                    //strcpy(space->message[msglength].str,"in the main window");                 
                    //写入未读消息
                    int fd, unread = 0;
                    char buff[20];
                    snprintf(buff, sizeof(buff), "%d", temp);
                    strcat(buff,"news.dat");   
                    fd = open(buff,O_RDWR|O_CREAT,0660);
                    read(fd,&unread,sizeof(int));
                    Data data_news;
                    Data data_fin;
                    for(int cntt = 0;cntt<unread;++cntt)
                    {
                        read(fd,&data_news,sizeof(data_news));
                    }                    
                    strcpy(data_news.message.str,"new_message");
                    strcpy(data_news.message.id_from, data.message.id_from);                    
                    strcpy(data_news.message.id_to, data.message.id_to); 
                    strcpy(data_fin.message.str, data.message.str);
                    strcpy(data_fin.message.id_from, data.message.id_from);                    
                    strcpy(data_fin.message.id_to, data.message.id_to);                    
                    //写一条（只保留一条）
                    ...
                    char path[20] = "history/";
                    char buffer[20];
                    snprintf(buff, sizeof(buff), "%d", fork_id);
                    strcat(buff,"to"); 
                    snprintf(buffer, sizeof(buffer), "%d", temp);
                    strcat(buff,buffer);
                    strcat(buff,".dat");
                    ...
                    printf("写入path：%s\n",path);
                    int fd1;
                    fd1 = open(path,O_RDWR|O_CREAT,0660);
                    //strcpy(data_news.message.str,space->message[msglength].str);
                    unread = 0;
                    read(fd1,&unread,sizeof(int));
                    printf("test 写入前：unread:%d\n",unread);
                    for(int cntt = 0;cntt<unread;++cntt)
                    {
                        read(fd1,&data_news,sizeof(data_news));
                    }         
                    //strcpy(data_news.message.str,space->message[msglength].str);                     
                    printf("写入的str:%s  id_to:%s  id_from: %s\n",data_fin.message.str,data_fin.message.id_to,data_fin.message.id_from);
                    write(fd1,&data_fin,sizeof(data_fin));
                    unread++;
                    printf("test:unread = %d\n",unread);
                    lseek(fd1,0,SEEK_SET);
                    write(fd1,&unread,sizeof(int)); 
                    
                    Data test;
                    read(fd1,&test, sizeof(Data));
                    printf("立刻测试: str:%s, id_to %s,id_from:%s\n ",test.message.str,test.message.id_to,test.message.id_from);
                    

                    close(fd1);                     
                    //space->length++;
                }/*B在主界面，A在chatroom给B发消息*/
                else if(space -> fwxnp[temp] == fork_id)
                {
                    //space->fwxnp[fork_id] = temp;
                    
                    printf("fork_id = %d, space -> fwxnp[fork_id] = %d space->fwxnp[temp] = %d\n",fork_id,space -> fwxnp[fork_id],space->fwxnp[temp]);
                ...
                else
                {
                    ...
                    //fwx
                    //只存入对应的history，发送的利用共享缓冲区结构体的方法实现
                    Data data_fin;
                    strcpy(data_fin.message.str, data.message.str);
                    strcpy(data_fin.message.id_from, data.message.id_from);                    
                    strcpy(data_fin.message.id_to, data.message.id_to);                     
                    ...
                    //strcpy(data_news.message.str,space->message[msglength].str);                     
                    printf("test::写入的str:%s  id_to:%s  id_from: %s\n",data_fin.message.str,data_fin.message.id_to,data_fin.message.id_from);
                    write(fd1,&data_fin,sizeof(data_fin));
                    unread++;
                    printf("test::    test:unread = %d\n",unread);
                    lseek(fd1,0,SEEK_SET);
                    write(fd1,&unread,sizeof(int)); 
                    
                    Data test;
                    read(fd1,&test, sizeof(Data));
                    printf("test 立刻测试: str:%s, id_to %s,id_from:%s\n ",test.message.str,test.message.id_to,test.message.id_from);
                    close(fd1);                    
            ...
            {
                printf("docuuuuu\n");
                printf("from %d to %d in %d\n",fork_id,space->fwxnp[fork_id],space->fwxnp[space->fwxnp[fork_id]]);
                if(1)
                {
                    strcpy(data.message.str,"1");//send
                    if(build_packet(&packet,enum_docu,data.message) == -1)
                    {
                        printf("docu fail to build the packet!\n");
                        return;
                    }
                    write(client_socket,&packet,sizeof(Packet));
                    ...
                    memset(iddd,0,sizeof(iddd));
                    sprintf(iddd,"%d",fork_id);
                    strcpy(filename,iddd);

                    strcat(filename,"to");

                    memset(iddd,0,sizeof(iddd));
                    sprintf(iddd,"%d",space->fwxnp[fork_id]);
                    strcat(filename,iddd);

                    strcat(filename,"_");
                    strcat(filename,data.message.str);
                    printf("file_name=%s\n",filename);

                    docu_rece(filename);

                    space->unsenddocu[space->doculen].id_to=space->fwxnp[fork_id];
                    space->unsenddocu[space->doculen].id_from=fork_id;
                    strcpy(space->unsenddocu[space->doculen].filename,filename);
                    strcpy(space->unsenddocu[space->doculen].name_from,fwxnb);
                    space->doculen+=1;
                }
            }
                //xzb
...
void write_to()
{
    ...
    while(1){
        ...
        //xzb
        for(int i=0;i<space->doculen;i++)
        {
            if(space->unsenddocu[i].id_to==fork_id)
            {
                Data data;
                strcpy(data.message.str,space->unsenddocu[i].filename);
                strcpy(data.message.id_from,space->unsenddocu[i].name_from);
                printf("%s is going to get a document named %s\n",fwxnb,space->unsenddocu[i].filename);
                if(build_packet(&packet,enum_docu,data.message) == -1)
                {
                    printf("docu fail to build the packet!\n");
                    return;
                ...
                write(client_socket,&packet,sizeof(Packet));
                if(docu_send(space->unsenddocu[i].filename)==1)
                {
                    printf("send success!!!\n");
                }
                else
                ...
                space->unsenddocu[i].id_to=0;
            }
        }
        //xzb
        temp = update_unread();
        //printf("test 执行完\n");
        temp = send_unread_message();
        if(temp>0)
        {
            printf("writ_to里面 当前id：%d account:%s, unread_message条数:%d\n", fork_id, fwxnb, temp);        
        }
...
void do_server()
{
    ...
    return_fri();
    int tt = update_unread();
    if(tt>0)
    {
        printf("test tt = %d 当前account：%s\n",tt,fwxnb);
    }
    ...
    switch(kind)
    {
        case enum_chat:
            ...
            strcpy(chat_with ,data.message.id_to);

            strcpy(data.message.str,"chat_in");
            if(build_packet(&packet,enum_chat,data) == -1)
            {
                printf("fail to build the packet!\n");
            }
        ...
        case enum_ipchat:
            ...
            write(client_socket,&packet,sizeof(Packet));
            space->fwxnp[fork_id] = 21;
            pthread_create(&thIDr, NULL,(void *)read_all,NULL);
            pthread_create(&thIDw, NULL,(void *)write_all,NULL);
            pthread_join(thIDr,NULL);
            goto k;
            //xzb
        case enum_friend:
