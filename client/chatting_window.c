...
void sendtouser(GtkButton  *button, gpointer entry)
{
	...
	else
	{
		...
		{	//比较but与"--发送--"，若相同返回0；若相同。
			...
			Data data1;
			Kind kind1;
			parse_packet(packet,&kind1,&data1);
			printf("id_to:%s	id_from:%s	text:%s kind:%d\n",data1.message.id_to,data1.message.id_from,data1.message.str,kind1);
		}
		else
		{
			strcat(data.message.str,text);
			if(build_packet(&packet,enum_chat_together,data) == -1){	    //打包类型为enum_chat的包
				printf("fail to build the packet!\n");
				return;
			}
			write(client_socket,&packet,sizeof(Packet));
